...
if [ "$helm_version" != "$supported_helm_version" ]; then
    ...
    echo "Please check https://github.com/dmotsenyat/helm-push-artifactory-plugin to find the correct version"
    exit 1
fi
...
url="https://github.com/dmotsenyat/helm-push-artifactory-plugin/releases/download/v${version}/${filename}"
